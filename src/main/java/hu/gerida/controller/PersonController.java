...
public class PersonController {
        ...
        private static final String KID = "GYEREK";
	...
	public List<Person> createCamper(@RequestBody String request, String name, String gender, String birthDate,
		...
		try {
			...
			Person person = new Person(name, Gender.valueOf(gender), textToDate, Planet.valueOf(planet), House.valueOf(house), Position.valueOf(pos), Integer.parseInt(size), foodSensitivity, other,
					cList, parent);
	...
	public List<Person> createCamper(@RequestBody String request){
		...
		//create more json object from the read object
		for(String key : requestObj.keySet()){
			String object = requestObj.get(key).toString();	//Jsonvalue to string
			jsonReader = Json.createReader(new StringReader(object));	//String to jsonReader
			objects.put(key, jsonReader.readObject()); //add new json object and its name to list
		}
		...
		Position pos = Position.valueOf(PersonController.KID);
		House house = House.valueOf(PersonController.NONE);
		Planet planet = Planet.valueOf(PersonController.NONE);
	...
	public List<Person> deleteCamper(@PathVariable("id") int id){
        ...
        /**
         * If {@code content} contains {@code ch} string, remove it from {@code content}.
         * @param content
         * @param ch
         * @return the content without ch
         */
	private String cut(String content, String ch){
        ...
        /**
         * If {@code content} contains {@literal "} string, remove it from {@code content}.
         * @param content
